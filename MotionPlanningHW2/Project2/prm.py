...
class StraightLinePlanner:
    ...
    def plan(self, start, goal):
        ...
        '''
        # Everything below in this method is generated by ChatGPT
        ...
        steps = int(np.ceil(distance / self.epsilon))

        for i in range(1, steps + 1):
            intermediate_point = start + i * self.epsilon * direction
            if self.in_collision(intermediate_point):
                return False
        return True
...
class PRM:
    ...
    def build_prm(self, reset=False):
        ...
        # Everything below in this method was generated by chatGPT.
        samples = []
        ...
        for sample in samples:
            node = RoadMapNode(sample)
            neighbors = self.find_valid_neighbors(node, samples, self.r)
            self.T.add_node(node, neighbors)
    ...
    def find_valid_neighbors(self, n_query, samples, r):
        ...
        # Everything below in this method was generated by chatGPT.
        for sample in samples:
            if np.linalg.norm(n_query.state - sample) < r:
                neighbor = RoadMapNode(sample)
                if self.local_planner.plan(n_query.state, sample):
    ...
    def query(self, start, goal):
        ...
        # Everything below in this method was generated by chatGPT up until said otherwise.
        start_neighbors = self.find_valid_neighbors(start_node, [n.state for n in self.T.nodes], self.r)
        goal_neighbors = self.find_valid_neighbors(goal_node, [n.state for n in self.T.nodes], self.r)
    ...
    def graph_search(self, init_node, is_goal):
        ...
        Perform graph search on the roadmap using uniform cost search.
        """
        return self.uniform_cost_search(init_node, is_goal)
    ...
    def uniform_cost_search(self, init_node, is_goal):
        ...
        # Everything below in this method is gerated by chatGPT
        frontier = PriorityQ()
        ...
        while len(frontier) > 0:
            node = frontier.pop()
            visited[tuple(node.state.tolist())] = node

            if is_goal(node.state):
                return backpath(node), visited

            for neighbor in node.neighbors:
                new_cost = node.cost + np.linalg.norm(node.state - neighbor.state)
                if tuple(neighbor.state.tolist()) not in visited or new_cost < visited[tuple(neighbor.state.tolist())].cost:
    ...
    def sample(self):
        ...
        return np.random.uniform(self.limits[:, 0], self.limits[:, 1], self.n)
...
def test_prm_env(num_samples=50, step_length=1, env='./env0.txt'):
    ...
    pe.draw_plan(None, prm,True,True,True)
...
if __name__== "__main__":
  test_prm_env(50, 1, './project2/env1.txt')
